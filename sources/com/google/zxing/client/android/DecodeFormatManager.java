...
final class DecodeFormatManager {
    ...
    static Set<BarcodeFormat> parseDecodeFormats(Uri uri) {
        Iterable queryParameters = uri.getQueryParameters(Scan.FORMATS);
        if (!(queryParameters == null || queryParameters.size() != 1 || queryParameters.get(0) == null)) {
            queryParameters = Arrays.asList(COMMA_PATTERN.split((CharSequence) queryParameters.get(0)));
        }
        return parseDecodeFormats(queryParameters, uri.getQueryParameter(Scan.MODE));
    }
