...
public final class CaptureActivityHandler extends Handler {
    ...
    public void handleMessage(Message message) {
        String str = null;
        if (message.what == C0306R.id.restart_preview) {
            restartPreviewAndDecode();
        } else if (message.what == C0306R.id.decode_succeeded) {
            float f;
            Bitmap bitmap;
        ...
        } else if (message.what == C0306R.id.decode_failed) {
            this.state = State.PREVIEW;
            this.cameraManager.requestPreviewFrame(this.decodeThread.getHandler(), C0306R.id.decode);
        } else if (message.what == C0306R.id.return_scan_result) {
            this.activity.setResult(-1, (Intent) message.obj);
            this.activity.finish();
        } else if (message.what == C0306R.id.launch_product_query) {
    ...
    public void quitSynchronously() {
        ...
        Message.obtain(this.decodeThread.getHandler(), C0306R.id.quit).sendToTarget();
        try {
            this.decodeThread.join(500);
        } catch (InterruptedException e) {
        }
        removeMessages(C0306R.id.decode_succeeded);
        removeMessages(C0306R.id.decode_failed);
    }
