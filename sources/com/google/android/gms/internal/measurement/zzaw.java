...
public class zzaw {
    ...
    private zzaw(zzay zzay) {
        ...
        this.zzwc = zzcm;
        zzau zzcp = new zzcp(this);
        zzcp.zzq();
        this.zzwe = zzcp;
        zzat zzby = zzby();
        String str = zzav.VERSION;
        zzby.zzs(new StringBuilder(String.valueOf(str).length() + 134).append("Google Analytics ").append(str).append(" is starting up. To enable debug logging on a device run:\n  adb shell setprop log.tag.GAv4 DEBUG\n  adb logcat -s GAv4").toString());
        zzcp = new zzct(this);
        zzcp.zzq();
        this.zzwj = zzcp;
        zzcp = new zzdh(this);
        zzcp.zzq();
        this.zzwi = zzcp;
        zzcp = new zzal(this, zzay);
        zzau zzbo = new zzbo(this);
        zzau zzak = new zzak(this);
        zzau zzbh = new zzbh(this);
        zzau zzcb = new zzcb(this);
        ...
        if (zzwb == null) {
            synchronized (zzaw.class) {
                if (zzwb == null) {
                    ...
                    zzwb = zzaw;
                    GoogleAnalytics.zzr();
                    elapsedRealtime = instance.elapsedRealtime() - elapsedRealtime;
                    long longValue = ((Long) zzcf.zzaal.get()).longValue();
                    if (elapsedRealtime > longValue) {
                        zzaw.zzby().zzc("Slow initialization (ms)", Long.valueOf(elapsedRealtime), Long.valueOf(longValue));
                    }
                }
            }
        }
