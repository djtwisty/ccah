...
final class zas implements zabs {
    ...
    public static zas zaa(Context context, zaaw zaaw, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<AnyClientKey<?>, Client> map, ClientSettings clientSettings, Map<Api<?>, Boolean> map2, AbstractClientBuilder<? extends zad, SignInOptions> abstractClientBuilder, ArrayList<zaq> arrayList) {
        ...
        for (Entry entry : map.entrySet()) {
            ...
            if (client2.requiresSignIn()) {
                c0238a.put((AnyClientKey) entry.getKey(), client2);
            } else {
                c0238a2.put((AnyClientKey) entry.getKey(), client2);
            }
        }
        Preconditions.checkState(!c0238a.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        Map c0238a3 = new C0238a();
        Map c0238a4 = new C0238a();
        for (Api api : map2.keySet()) {
            AnyClientKey clientKey = api.getClientKey();
            if (c0238a.containsKey(clientKey)) {
                c0238a3.put(api, (Boolean) map2.get(api));
            } else if (c0238a2.containsKey(clientKey)) {
                c0238a4.put(api, (Boolean) map2.get(api));
            } else {
        ...
        while (i < size) {
            ...
            i++;
            zaq zaq = (zaq) obj;
            if (c0238a3.containsKey(zaq.mApi)) {
                arrayList2.add(zaq);
            } else if (c0238a4.containsKey(zaq.mApi)) {
                arrayList3.add(zaq);
            } else {
                throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
            }
        }
        return new zas(context, zaaw, lock, looper, googleApiAvailabilityLight, c0238a, c0238a2, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, c0238a3, c0238a4);
    }
    ...
    private zas(Context context, zaaw zaaw, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<AnyClientKey<?>, Client> map, Map<AnyClientKey<?>, Client> map2, ClientSettings clientSettings, AbstractClientBuilder<? extends zad, SignInOptions> abstractClientBuilder, Client client, ArrayList<zaq> arrayList, ArrayList<zaq> arrayList2, Map<Api<?>, Boolean> map3, Map<Api<?>, Boolean> map4) {
        ...
        this.zabj = looper;
        this.zaei = client;
        this.zaee = new zabe(context, this.zaed, lock, looper, googleApiAvailabilityLight, map2, null, map4, null, arrayList2, new zau());
        this.zaef = new zabe(context, this.zaed, lock, looper, googleApiAvailabilityLight, map, clientSettings, map3, abstractClientBuilder, arrayList, new zav());
        Map c0238a = new C0238a();
        for (AnyClientKey put : map2.keySet()) {
            c0238a.put(put, this.zaee);
        }
        for (AnyClientKey put2 : map.keySet()) {
            c0238a.put(put2, this.zaef);
        }
        this.zaeg = Collections.unmodifiableMap(c0238a);
    }
