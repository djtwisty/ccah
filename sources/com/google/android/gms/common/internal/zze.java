...
final class zze extends GmsClientSupervisor implements Callback {
    ...
    protected final boolean zza(zza zza, ServiceConnection serviceConnection, String str) {
        ...
        synchronized (this.zzdu) {
            ...
            if (this.zzdu.size() > HttpStatus.SC_OK) {
                String valueOf2 = String.valueOf(zza);
                Log.wtf("GmsClientSupervisor", new StringBuilder(String.valueOf(valueOf2).length() + 51).append("Adding ").append(valueOf2).append(", and there are now ").append(this.zzdu.size()).append(" connections.").toString(), new Exception());
            }
            if (zzf.zzs() > 100) {
                int zzs = zzf.zzs();
                String valueOf3 = String.valueOf(zza);
                Log.wtf("GmsClientSupervisor", new StringBuilder(String.valueOf(valueOf3).length() + 41).append("There are").append(zzs).append(" clients for service ").append(valueOf3).toString(), new Exception());
            }
            isBound = zzf.isBound();
        }
        return isBound;
            ...
            case 1:
                synchronized (this.zzdu) {
                    ...
                    if (zzf != null && zzf.getState() == 3) {
                        ...
                        if (componentName2 == null) {
                            componentName = new ComponentName(zza.getPackage(), NetworkManager.TYPE_UNKNOWN);
                        } else {
